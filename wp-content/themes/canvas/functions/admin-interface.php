...
function woothemes_add_admin() {
	...
	elseif ( isset($_REQUEST['page']) && $_REQUEST['page'] == 'woothemes_sbm' ) {
		if (isset($_REQUEST['woo_save']) && 'sbm_reset' == $_REQUEST['woo_save']) {
	...
	if (get_option('woo_needs_tumblog_upgrade') == 'true') {
		if (function_exists('woo_upgrade_existing_tumblog_posts')) {
			$wootumblog = add_submenu_page('woothemes', 'Tumblog Upgrade', 'Tumblog Upgrade', 8, 'woothemes_tumblog', 'woothemes_tumblog_upgrade_page');
		}
	}
	...
	if (get_option('woo_needs_tumblog_upgrade') == 'true') {
		if (function_exists('woo_upgrade_existing_tumblog_posts')) {
			add_action("admin_print_scripts-$wootumblog", 'woo_load_only');
		}
	} 
...
function woo_ajax_callback() {
	...
	if($save_type == 'upload'){
		...
		$uploaded_file = wp_handle_upload($filename,$override);
				...
				//ADD ATTACHMENT - Work in Progress
				//$wp_filetype = wp_check_filetype($uploaded_file['file']);
				/*$attachment = array(
				...
				//$attach_data = wp_generate_attachment_metadata( $attach_id, $filename['name'] );
...
function woothemes_machine($options) {
	...
	foreach ($options as $value) {
		...
		case "typography":
			...
			$output .= '<option value="Cuprum, Arial, serif" '. $font34 .'>Cuprum</option>';
			$output .= '<option value="Neucha, Arial, serif" '. $font35 .'>Neucha</option>';
			$output .= '<option value="Neuton, Arial, serif" '. $font36 .'>Neuton</option>';
			$output .= '<option value="PT Sans, Arial, serif" '. $font37 .'>PT Sans</option>';
			$output .= '<option value="Philosopher, Arial, serif" '. $font38 .'>Philosopher</option>';
