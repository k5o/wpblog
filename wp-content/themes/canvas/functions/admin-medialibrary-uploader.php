...
if ( is_admin() ) {
	...
	if ( ( isset( $_REQUEST['page'] ) && substr( $_REQUEST['page'], 0, 3 ) == 'woo' ) || $is_posts_page ) {
...
if ( ! function_exists( 'woothemes_mlu_js' ) ) {
	...
	function woothemes_mlu_js () {
		...
		wp_register_script( 'woo-medialibrary-uploader', get_bloginfo('template_directory').'/functions/js/woo-medialibrary-uploader.js', array( 'jquery', 'thickbox' ) );
...
if ( ! function_exists( 'woothemes_medialibrary_uploader' ) ) {
	...
	function woothemes_medialibrary_uploader ( $_id, $_value, $_mode = 'full', $_desc = '', $_postid = 0 ) {
		...
		// echo 'DEBUG: $_id: ' . $id . ' $_value: ' . $value . ' $_mode: ' . $_mode . ' $_desc: ' . $_desc . ' $_postid: ' . $int . '<br /><br />'; // DEBUG
		...
		if ( $value != '' ) { 
			...
			if ( $image ) {
				...
				// $output .= '<div class="no_image"><a href="' . $value . '">'.$title.'</a>' . $remove . '</div>';
