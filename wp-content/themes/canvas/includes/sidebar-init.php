...
if (!function_exists('the_widgets_init')) {
	function the_widgets_init() {
		...
		if (!$total)
			$total = 4;
		$i=0; while ($i < $total) : $i++;
			register_sidebar(array('name' => 'Footer '.$i,'id' => 'footer-'.$i, 'description' => "Widetized footer", 'before_widget' => '<div id="%1$s" class="widget %2$s">','after_widget' => '</div>','before_title' => '<h3>','after_title' => '</h3>'));
		endwhile;
