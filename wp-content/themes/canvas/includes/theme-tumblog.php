...
function woo_tumblog_audio_content($post_id) {
	...
	if(!empty($link_url)) {
		...
		$content_tumblog .= '<script type="text/javascript">
		      				var so = new SWFObject("'.get_bloginfo('template_directory').'/includes/player.swf","mpl","'.get_option('woo_tumblog_audio_width').'","32","9");
		      				so.addParam("allowfullscreen","true");
		      				so.addParam("allowscriptaccess","always");
		      				so.addParam("wmode","opaque");
		      				so.addParam("wmode","opaque");
		      				so.addVariable("skin", "'.get_bloginfo('template_directory').'/includes/stylish_slim.swf");
...
function woo_tumblog_the_title($outer_element = 'h1', $class = 'title', $icon = true, $before = '', $after = '', $return = false) {
    ...
    foreach ($tumblog_array as $tumblog_item) {
      	...
      	if ( $tumblog_items['articles'] == $tumblog_id->term_id && !$sentinel ) {
      		...
      		$content = $icon_content.$before.'<'.$outer_element.' class="'.$class.'"><a href="'.get_permalink($post_id).'" title="'.esc_attr__( "Permalink to ".get_the_title($post_id), "woothemes" ).'" rel="bookmark">'.get_the_title($post_id).'</a></'.$outer_element.'>'.$after;
       	} elseif ($tumblog_items['images'] == $tumblog_id->term_id && !$sentinel ) {
       		if ($icon) {
       		...
       		$content = $icon_content.$before.'<'.$outer_element.' class="'.$class.'"><a href="'.get_post_meta($post_id,'link-url',true).'" title="'.esc_attr__( "Permalink to ".get_the_title($post_id), "woothemes" ).'" rel="bookmark" target="_blank">'.get_the_title($post_id).'</a></'.$outer_element.'>'.$after;
       	} else {
       		$content = $before.'<'.$outer_element.' class="'.$class.'"><a href="'.get_permalink($post_id).'" title="'.esc_attr__( "Permalink to ".get_the_title($post_id), "woothemes" ).'" rel="bookmark">'.get_the_title($post_id).'</a></'.$outer_element.'>'.$after;
      	}	    		
    } 
