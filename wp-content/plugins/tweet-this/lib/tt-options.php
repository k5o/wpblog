<?php
/**
 * Tweet This is a plugin for WordPress 2.7 - 3.0.5 and WordPress MU.
 ...
 */
	...
	echo	'><div class="option"><label for="tt[tt_bitly_username]">' .
		__('Bit.ly Username (MANDATORY)', 'tweet-this') . ':</label>' .
		...
		__('Bit.ly API Key (MANDATORY)', 'tweet-this') . ':</label>' .
	...
	echo	'" /><p style="width:800px;padding-bottom:4px;">' .
		...
		'prefix, <a href="http://tinyurl.com/api-create.php?url=http://wordpress.org/extend/plugins/tweet-this/">like this</a>. The ' .
	...
	echo	'<div><p><strong>' . __('Shortcodes available in Twitter ' .
		...
		'</strong></p><p>[TITLE] [TITLE_SHARE] [URL] [AUTHOR] ' .
		'[CATEGORY] [DATE] [TIME] [EXCERPT] [BLOG_TITLE]</p></div>' .
	...
	// OAuth replaced Basic Auth in v1.7.
		...
		'</strong> below.</p>' .
		'<p><font color="red"><strong><em>Step 4</em></strong>: ' .
		'Click "My Access Token" on the right, and then, copy and ' .
		'paste <strong>Access Token</strong> and <strong>Access ' .
		'Token Secret</strong> below.</font></p>' .
	...
	echo	'" /></p><p>' . __('CSS to insert or [BLANK] for none. ' .
		'Use [IMG_CLASS] and [LINK_CLASS] for the variables above.',
	...
	echo	'" /></p><p><label>' .
		'<input type="checkbox" name="tt[tt_insert_before]"';
	if(tt_option('tt_insert_before') == 'true')
		echo	$v;
	echo	' /> ' . __('Insert Tweet This block before content (can be ' .
		'combined with below).', 'tweet-this') . '</label></p><p>' .
		'<label><input type="checkbox" name="tt[tt_insert_after]"';
	if(tt_option('tt_insert_after') != 'false')
		echo	$v;
	echo	' /> ' . __('Insert Tweet This block after content (can be ' .
		'combined with above).', 'tweet-this') . '</label></p><p>' .
		'<label><input type="checkbox" name="tt[tt_shortlink_filter]"';
	...
	if(tt_option('tt_limit_to_single') == 'true')
		echo	$v;
	echo	' /> ' . __('Only show Tweet This block when viewing single ' .
		'posts or pages.', 'tweet-this') . '</label></p><p><label>' .
		'<input type="checkbox" name="tt[tt_limit_to_posts]"';
	if(tt_option('tt_limit_to_posts') == 'true')
		echo	$v;
	echo	' /> ' . __('Hide Tweet This block on pages (will override ' .
		'checkbox above).', 'tweet-this') . '</label></p><p><label>' .
		'<input type="checkbox" name="tt[tt_hide_on_feeds]"';
	if(tt_option('tt_hide_on_feeds') == 'true')
		echo	$v;
	echo	' /> ' . __('Hide Tweet This block on all feeds (RSS, RDF, ' .
		'Atom).', 'tweet-this') . '</label></p><p><label>' .
	...
	echo	' /> ' . __('Insert Tweet This message in footer.',
		'tweet-this') . '</label></p>><p><em>' . __('Some ' .
		'advanced options can only be set in the tt-config.php file ' .
		'in the tweet-this directory.', 'tweet-this') . '</em></p>' .
		'<div style="width:800px;padding-top:0;padding-bottom:0;' .
	...
	echo	'</div><div style="width:800px;padding-top:0;padding-' .
		...
		'tt[tt_limit_to_posts], tt[tt_hide_on_feeds], ' .
		'tt[tt_auto_tweet_display], ' .
		...
		'tt[tt_new_window], tt[tt_nofollow], ' .
	...
	echo	'<div style="width:800px;padding-top:4px;padding-bottom:0;">' .
		...
		'Please donate <a target="_blank" href="https://' .
		...
		'plugin">$11.90 or more</a> or <a target="_blank" href=' .
		'"https://www.paypal.com/cgi-bin/webscr?cmd=_xclick&amp;' .
		'business=thripp@gmail.com&amp;currency_code=USD&amp;amount=' .
		'&amp;return=&amp;item_name=Donations+for+Tweet+This+' .
		'plugin">less than $11.90</a>. <a target="_blank" href=' .
